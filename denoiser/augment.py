...
from denoiser.dsp import mel_frequencies, LowPassFilters
...
class BandMask(nn.Module):
    ...
    def forward(self, wav):      
        ...
        # # patch
        # noise_masked, clean_masked = out
        # out = torch.stack([clean_masked - clean_init + noise_masked, clean_init])
...
class Shift(nn.Module):
    ...
    def forward(self, wav):
        ...
        if self.shift > 0:            
            ...
            # patch
            noise_offset, clean_offset = offsets
            noise_offset_flatten = torch.flatten(noise_offset)
            clean_offset_flatten = torch.flatten(clean_offset)
            
            # handle noise
            noise_shifted = list()
            for i in range(noise.shape[0]):
                indexes = torch.roll(torch.arange(length, device=wav.device), int(noise_offset_flatten[i]))
                noise_shifted.append(noise[i].gather(-1, indexes.unsqueeze(0)))
            noise_shifted_batch = torch.stack(noise_shifted)
            
            # handle clean
            clean_shifted = list()
            for i in range(clean.shape[0]):
                indexes = torch.roll(torch.arange(length, device=wav.device), int(clean_offset_flatten[i]))
                clean_shifted.append(clean[i].gather(-1, indexes.unsqueeze(0)))
            clean_shifted_batch = torch.stack(clean_shifted)

            wav = torch.stack([noise_shifted_batch, clean_shifted_batch])
        return wav
